...
def mock_duckdb_lib(mocker):
    ...
    mock_lib = mocker.patch(
        "data_scribe.components.db_connectors.duckdb_connector.duckdb"
    )
...
def test_duckdb_get_tables_db_file(mock_duckdb_lib: MagicMock):
    ...
    mock_duckdb_lib.mock_cursor.fetchall.return_value = [("table1",), ("view1",)]
    ...
    mock_duckdb_lib.mock_cursor.execute.assert_called_once_with("SHOW ALL TABLES;")


def test_duckdb_get_tables_directory_scan(mock_duckdb_lib: MagicMock):
    """Tests get_tables for a local directory scan."""
    mock_duckdb_lib.mock_cursor.fetchall.return_value = [("users.parquet",), ("orders.csv",)]
...
def test_duckdb_get_tables_s3_scan(mock_duckdb_lib: MagicMock):
    ...
    # Should use 's3_glob'
    expected_query = "SELECT basename(file_name) FROM s3_glob('s3://my-bucket/data/*.*')"
    # Call list includes httpfs install + this glob query
    assert mock_duckdb_lib.mock_cursor.execute.call_args_list[1] == call(expected_query)
...
def test_duckdb_get_columns_file_scan(mock_duckdb_lib: MagicMock):
    ...
    mock_duckdb_lib.mock_cursor.fetchall.return_value = [("id", "INTEGER"), ("email", "VARCHAR")]
    
    connector = DuckDBConnector()
    connector.connect({"path": "s3://my-bucket/data/"}) # Directory scan mode
    columns = connector.get_columns("users.parquet") # table_name is just the file name

    assert columns == [{"name": "id", "type": "INTEGER"}, {"name": "email", "type": "VARCHAR"}]
...
def test_duckdb_get_column_profile_file_scan(mock_duckdb_lib: MagicMock):
    ...
    mock_duckdb_lib.mock_cursor.fetchone.return_value = (100, 10, 90) # total, null, distinct
    ...
    normalized_actual = " ".join(mock_duckdb_lib.mock_cursor.execute.call_args[0][0].split())
